...
function createInviteCode($email){
    ...
    $headers .= "Reply-To: noreply@mp3act.net\n";
			...
			$msg .= "http://$GLOBALS[http_server]$GLOBALS[uri_path]/register.php?invite=$code";
			if(mail($email,'Invitation to Join an mp3act Server',$msg,$headers)){
			  return 1;
			}
		}
}
  	...
  	if(mysql_num_rows($result) == 0){
  		...
  		$msg = "$email,\n\nYou have requested a new password for the mp3act server you are a member of. Your password has been reset to a new random password. When you login please change your password to a new one of your choice.\n\n";
			...
			$headers  = "";
			    ...
			    $headers .= "From: \"mp3act server\" <noreply@mp3act.net>\n";
			    $headers .= "Reply-To: noreply@mp3act.net\n";
			mail($email,'Your Password for mp3act',$msg,$headers);
  		return 1;
  	}
...
function buildSongList( $text, $result, $extra = '' ) {
  $count=1; $items = array(); $add='';
  while($row = mysql_fetch_array($result)){
    $alt = getAlt($count);
    $id = "song$row[song_id]";
    if ( $extra='numplays' ) { $add = "<small>" . $row['numplays'] . "Plays</small> "; }
  ...
  if ( $count == 1 ) return $text . "<p>No songs found</p>";
  $text = "<ul> " . $text . "</ul>\n";
  $output = array();
  $output[] = $text;
  $output[] = $count-1;
...
function buildAlbumList( $text, $query, $extra='' ) {
  ...
  while($row = mysql_fetch_array($result)){
    ...
    if ( $extra == 'pubdate') { $add = "<small>" . $row['pubdate'] . "</small> "; }
...
function adminAddUser($firstname='',$lastname='',$username='',$email='',$level='',$pass=''){
      if(!empty($firstname)){
        ...
        $query = "INSERT INTO mp3act_users VALUES 
        							...
        							PASSWORD(\"".$pass."\"),$level,NOW(),1,\"".$email."\",\"streaming\",0,\"s\",\"$md5\",\"\",\"\",1)";
        if(mysql_query($query)){
          return 1;
        }
...
function editUser($type,$input1,$input2,$input3,$input4){
	mp3act_connect();
	switch($type){
	case 'info':
			if(!empty($input1)){
	...
	case 'settings':
			if(!empty($input1)){
				$query = "UPDATE mp3act_users SET default_mode=\"$input1\",default_bitrate=$input2,default_stereo=\"$input3\",theme_id=$input4 WHERE user_id=$_SESSION[sess_userid]";
				mysql_query($query);
				return 1;
			}
			$query = "SELECT * FROM mp3act_users WHERE user_id=$_SESSION[sess_userid]";
...
function viewPlaylist(){
	...
	$text='';
	$items = array();
	$query = "SELECT mp3act_playlist.*, mp3act_artists.artist_name,mp3act_artists.prefix, mp3act_songs.name,mp3act_albums.album_name,mp3act_songs.track,SEC_TO_TIME(mp3act_songs.length) AS time FROM mp3act_playlist,mp3act_artists,mp3act_songs,mp3act_albums WHERE mp3act_playlist.song_id=mp3act_songs.song_id AND mp3act_artists.artist_id=mp3act_songs.artist_id AND mp3act_songs.album_id=mp3act_albums.album_id AND " . playlistCondition() . " ORDER BY mp3act_playlist.pl_id";
	$result=mysql_query($query);
	while($row = mysql_fetch_array($result)){
	  $id = "pl{$row[pl_id]}";
...
function savePlaylist($pl_name, $prvt){
	...
	$songs = array();
	$time=0;
	$query = "SELECT mp3act_playlist.song_id,mp3act_songs.length FROM mp3act_playlist,mp3act_songs WHERE mp3act_songs.song_id=mp3act_playlist.song_id AND " . playlistCondition() . " ORDER BY mp3act_playlist.pl_id";
	$result = mysql_query($query);
	while($row = mysql_fetch_array($result)){
		$songs[] = $row['song_id'];
		$time += $row['length'];
...
function playlistInfo(){
	mp3act_connect();
	$query = "SELECT COUNT(mp3act_playlist.pl_id) as count, SEC_TO_TIME(SUM(mp3act_songs.length)) as time FROM mp3act_playlist,mp3act_songs WHERE mp3act_playlist.song_id=mp3act_songs.song_id AND " . playlistCondition();
	$result = mysql_query($query);
	$row = mysql_fetch_array($result);
	if($row['count'] == 0){
		return "Playlist is empty";
...
function playlistCondition() {
  return " " . (($_SESSION['sess_playmode'] == "streaming") ? ("mp3act_playlist.user_id=" . $_SESSION['sess_userid'] . " AND private=1 ") : ("private=0 "));
}

// playlist is an encoded string with playlist items
function playlist_update( $playlist ) {
	mp3act_connect();
	$query = "SELECT song_id FROM mp3act_playlist WHERE " . playlistCondition() . " ORDER BY pl_id";
	$result = mysql_query($query);
	while($row = mysql_fetch_array($result)){
	  $current[] = $row['song_id'];
	}
	...
	if ($_SESSION['sess_playmode'] != "streaming") { // renumber currently playing song
	  $query = "SELECT mp3act_currentsong.pl_id FROM mp3act_currentsong";
	  $result = mysql_query($query);
	...
	if ( $current_nr > count( $new ) ) {
	  $query = "DELETE FROM mp3act_playlist WHERE pl_id >=" . count($new) . " AND " . playlistCondition();
	  mysql_query($query);
	...
	for ( $i=0; $i<$current_nr; $i++) {
	  if ( $current[$i] != $new[$i] ) {
	    $query = "UPDATE mp3act_playlist SET song_id=$new[$i] WHERE pl_id={$i} AND " . playlistCondition();
	    mysql_query($query);
	  }
	}
	
	while ( $i < count($new) ) {
	    $query = "INSERT INTO mp3act_playlist VALUES($id, $new[$i], $_SESSION[sess_userid],".($_SESSION['sess_playmode'] == "streaming" ? 1 : 0).")";
	    mysql_query($query);
	    $i++;
	}
	...
	if ( isset( $currently_playing_pl_id ) ) {
	  ...
	  if ( $i<count($new)) { // found, restore pl_id
	    if ( $i!=$currently_playing_pl_id ) { // moved, so renumber to $i
	      $query = "UPDATE mp3act_currentsong SET mp3act_currentsong.pl_id={$i}";
	       mysql_query($query);
	    }
	  } else { // currently playing song has been removed from list
	    play ( "jukebox", "restart", 0 );
	  }
	}
}
...
function playlist_add($type,$itemid){
	...
	case 'song':	  
	  ...
	  $result = mysql_query($query);
	  $row = mysql_fetch_array($result); $row['pl_id'] = $id;
	...
	case 'album':
	  ...
	  while($row = mysql_fetch_array($result)){
	    ...
	    $output[] = "pl{$id}";
	    $row['pl_id'] = $id; $items .= createSongLI( $row );
	    $id++;
	  }
	  $text[] = $items;
	  $num[] = count($output);
  	...
  	header("Pragma: nocache"); 
		...
		$result = mysql_query($query);
			...
			while($row = mysql_fetch_array($result)){
				...
				$tmp .= "http://$GLOBALS[http_server]$GLOBALS[uri_path]/playstream.php?i=$row[song_id]&u=$_SESSION[sess_usermd5]&b=$_SESSION[sess_bitrate]&s=$_SESSION[sess_stereo]\n";
			}
		return $tmp;   
		exit;
	}
		...
		switch($type){
			case 'artists':
				if(!file_exists("/tmp/mp3act")){
   			 	 ...
   			 	 exec(getSystemSetting("phpbin")." includes/play.php 6 $string > /dev/null 2>&1 &"); 
					}
				break;
				case 'albums':
				if(!file_exists("/tmp/mp3act")){
   			 	 ...
   			 	 exec(getSystemSetting("phpbin")." includes/play.php 5 $string > /dev/null 2>&1 &"); 
					}
				break;
				case 'genre':
				if(!file_exists("/tmp/mp3act")){
   			 	 ...
   			 	 exec(getSystemSetting("phpbin")." includes/play.php 4 $string > /dev/null 2>&1 &"); 
					}
				break;
			case 'all':
				if(!file_exists("/tmp/mp3act")){
...
function play($mode,$type,$id){
	...
	// JUKEBOX MODE
			...
			switch($type){
			  // ivy: killall -c does not work, killall -Z warns about SELinux kernel requirement, but works
				case 'stop':
					exec("killall -Z ".basename(getSystemSetting("phpbin"))." > /dev/null 2>&1 &");
					exec("killall -Z ".basename(getSystemSetting("mp3bin"))." > /dev/null 2>&1 &");
					if(file_exists("/tmp/mp3act")){
						unlink("/tmp/mp3act");
					}
				...
				break;
				case 'prev':
					 $query = "UPDATE mp3act_currentsong SET pl_id=pl_id-2";
					 mysql_query($query);
					 exec("killall ".getSystemSetting("mp3bin")." > /dev/null 2>&1 &");
				break;
				case 'restart':
					 $query = "UPDATE mp3act_currentsong SET pl_id=pl_id-1";
					 mysql_query($query);
					 exec("killall ".getSystemSetting("mp3bin")." > /dev/null 2>&1 &");
				break;
				case 'next':
...
function playLocal($file){
	...
	// ivy: either add slashes, or add \"
	// $tmp=exec("$mp3player -k 2 \"".addslashes($file)."\" 2>&1");
	$tmp=exec("$mp3player -k 2 \"$file\" 2>&1");
}
...
function streamPlay($id, $rate=0, $stereo="s",$user='',$ip=''){
  ...
  if(verifyIP($user,$ip)){
  //$query="SELECT filename,size,length FROM mp3act_songs WHERE song_id=$id";
					...
					for ($i = 0; $i < $maxlinks2; $i++) {
							...
							if (strstr($artMedium, "amazon.com") == true) {
								 ...
								 /* $medium_size = getimagesize($artMedium);
									...
									//return $artSmall;  
									return $artMedium; 
									//$this->art_insert($art_locator, $artSmall, $small_okay, $artMedium, $medium_okay, $artLarge, $large_okay);
							
							} else {
															//echo $contentStart;    
									return '';
									//$this->art_insert($art_locator, "", "no", "", "no", "", "no");
