		...
		var other;
			...
			if ( other ) parent.insertBefore( parent.removeChild(item), other );
		} else if(direction == "down") {
			other = item.nextSibling;
			if ( other ) parent.insertBefore( parent.removeChild(other), item );
		}
		if ( other ) Fat.fade_element(other.id,null,900,'#ffcc99','#f3f3f3');
		Fat.fade_element(item.id,null,900,'#ffcc99','#f3f3f3');
		playlist.options.onUpdate('playlist');
   	}
    	...
    	function refresh_cb(new_data) {
			...
			function updateBox_cb(new_data){
				if ( new_data[1] > 0 ) { // how do I distinguish array and string?
					document.getElementById("info").innerHTML = new_data[0];
					for(var i=2; i<new_data[1]+2; i++){
					   	new Draggable(new_data[i],{revert:true});
					}
				} else {
			...
			function updateBox(type,itemid){
				...
				if ( type=='album' && just_added!='' && just_added==itemid ) { just_added=''; return; }
			...
			function plrem(item){
				playlist.remove( document.getElementById(item) );
				playlist.options.onUpdate('playlist');
			}
			...
			function playlist_update_cb( str ) {
				...
				for (var i = 0; i < list.length; i++) {
					list[i].setAttribute("id", "pl" + i);
					/* This does not work for fixing the black-box underflow bug:
					   var p = list[i].getElementsByTagName("p")[0];
					   p.style.zIndex = list.length+5-i;

					   neither does adding to css:
					   .box ul li:hover p{
					     z-index: 100; 
					   } 
					   maybe z-index manipulation in dragdrop.js can solve it
					  */
				}
			}
			...
			function pladd_cb(new_data){
				if(new_data[0] == 1){
					...
					r.setStartBefore(document.getElementById("playlist"));
					var node = r.createContextualFragment(new_data[0]);
					...
					for(var i=2; i<new_data[1]+2; i++){
						...
						playlist.add( document.getElementById(new_data[i]));
					}
					x_playlistInfo(plinfo_cb);
				}
				// alert("playlist: " + Sortable.serialize('playlist') );
			}
			...
			function editUser(type,form){
				if(form != 0){
					if(type == 'info'){
						...
						x_editUser(type,form.default_playmode.value,form.default_bitrate.value,form.default_stereo.value,form.theme_id.value,editUser_cb);
					}
					else if(type == 'pass'){
