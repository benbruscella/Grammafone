...
<channel>
	...
	<link><?php echo "http://$GLOBALS[http_server]$GLOBALS[uri_path]/"; ?></link>
	<description>A list of the 10 most recently added music albums to this mp3act server.</description>
	<generator><?php echo "http://$GLOBALS[http_server]$GLOBALS[uri_path]/feed.php"; ?></generator>
...
<description>	<![CDATA[<?php echo "<p ><strong>Artist:</strong> $row[prefix]".htmlentities($row['artist_name'])."<br/><strong>Album:</strong> ".htmlentities($row['album_name']); 
		if($row['album_art'] && $row['album_art'] != 'fail' ) { echo "<br/><img src=\"http://$GLOBALS[http_server]$GLOBALS[uri_path]/art/".$row['album_art']."\" />"; }
	?>
	</p>]]></description>
...
<content:encoded>
	<![CDATA[<?php echo "<p style=\"margin:0; border-bottom: 8px solid #aaa; border-top: 8px solid #aaa; padding: 8px; background: #ddd; \"><strong>Artist:</strong> $row[prefix] ".htmlentities($row['artist_name'])."<br/><strong>Album:</strong> ".htmlentities($row['album_name']); 
		if($row['album_art'] && $row['album_art'] != 'fail' ) { echo "<br/><img style=\"background: #fff; border:1px solid #999; margin:0; padding:3px; \" src=\"http://$GLOBALS[http_server]$GLOBALS[uri_path]/art/".$row['album_art']."\" />"; }
	?>
	</p>]]>
</content:encoded>
