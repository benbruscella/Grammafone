...
class addMusic {
  ...
  function setDisplayResults ($switch){
	...
	function outputProgressJavascript($progress)
	{
		echo "<script language=\"javascript\">\n";
		echo "p = document.getElementById(\"error\");";
		echo "p.innerHTML = 'Importing... <strong>[$progress%]</strong>';";
        echo "</script>\n";
		$this->flushDisplay();
	}
	...
	function & getSongs($path, & $filelist) {
		...
		if ($resdir != false) {
			while (($entry = readdir($resdir)) !== false) {
				...
				if (is_dir($path."/".$entry)) { //if it's a directory...
					...
					// if it is not a dir, read the filename
		...
		else
		{
			...
			echo "<li>Directory $path not open for reading, permission problem?</li>\n";
		}
	...
	/* import one song into the database, returns 1 or 0
	 * depending on whether it was added or not. note that fname
	 * must NOT be slashed, this function takes care of that! */
	function importOneSong($getID3, $fname)
	{
		/* we have a song name, get its access time and compare to the database */
		$imported = 0;
		$do_the_update = 0; 
		$flag = 0;

		$song_filename = addslashes($fname);
		$current_access_time = filemtime($fname);

		$query = "SELECT * FROM mp3act_songs WHERE filename=\"$song_filename\"";
		
		$mysql_result = mysql_query($query);
		if (mysql_num_rows($mysql_result) == 0) {
			/* the song was not in the database... do the update */
			$song_exists_in_database = 0;
			$do_the_update = 1; 
		}
		else
		{
			/* find the access time for the song and compare to current access time */
			$song_exists_in_database = 1;
		...
		{			
			$tagInfo = array ();
			$tagInfo = $getID3->analyze($fname);
			
			if (strcmp($tagInfo['error'], "") != 0)
			{
				echo "<li>Could not get tag info for $fname, possible reasons: permission problem, no tag info, or corrupted file.</li>\n";
			}
			else
			{
				$fileproblems = array();
				...
				if (!($goodData["name"] = addslashes(ucwords($tagInfo['tags']['id3v2']['title'][0])))) {
					if (!($goodData["name"] = addslashes(ucwords($tagInfo['tags']['id3v1']['title'][0])))) {
						$fileproblems[] = "Song title";
						$goodData["name"] = "Unknown";
						$flag = 1;
					}
				}
				# get track number
				...
				/* Upload this record into the database */
				
				if (!$flag)
				{
					/* Insert the artist if it doesn't exist yet */
					...
					/* Insert the genre if it doesn't exist yet */
					$query = "SELECT genre_id FROM mp3act_genres WHERE genre=\"$goodData[genre]\"";

					$result = mysql_query($query);
					$genre = mysql_fetch_array($result);
					if ($genre['genre_id'] > 0) {
						$current_genre = $genre['genre_id'];
					} else {
						$query = "INSERT INTO mp3act_genres VALUES (NULL,\"$goodData[genre]\")";

						$result = mysql_query($query);
						$current_genre = mysql_insert_id();
					}

					/* Insert the album name if it doesn't exist yet */
					...
					if ($album['album_id'] > 0) {
						...
						$query = "INSERT INTO mp3act_albums VALUES (NULL,\"$goodData[album]\",$current_artist,$current_genre,$goodData[year],\"\")";

						$result = mysql_query($query);
						$current_album = mysql_insert_id();
					}

					/* Insert the song data itself (might be a replace) */
					if ($song_exists_in_database == 1) {
						$query = "REPLACE mp3act_songs VALUES";
						$query .= " ($song_id,$current_artist,$current_album,$current_genre,NOW(),\"$goodData[name]\",$goodData[track],";
						$query .= "\"$goodData[length]\",$goodData[size],$goodData[bitrate],\"$goodData[type]\",0,";
						$query .= "\"$song_filename\",0,$current_access_time)";
					}
					else
					{
						$query = "INSERT INTO mp3act_songs VALUES";
						$query .= " (NULL,$current_artist,$current_album,$current_genre, NOW(),\"$goodData[name]\",$goodData[track],";
						$query .= "\"$goodData[length]\",$goodData[size],$goodData[bitrate],\"$goodData[type]\",0,";
						$query .= "\"$song_filename\",0,$current_access_time)";
					}
					...
					if (mysql_query($query)) {
						if ($this->displayResults == 1) {
							echo "<li>Added ".$goodData["name"]." By ".$goodData["artist"]."</li>\n";
						}
						$imported = 1;
					} else {
						echo "<li>File $fname could not be added to the database. See mysql error below:</li>\n";
						echo "<li>".mysql_error()."</li>\n";
						echo "<li>Tried to execute query: $query</li>\n";
					}
				}
				else
				{
					$missing = "Missing ";
					$idx = 0;
					while ($idx < count($fileproblems)) 
					{
						$missing .= $fileproblems[$idx++];
						if (count($fileproblems) - $idx == 0)
							$missing .= ".";
						else if (count($fileproblems) - $idx == 1)
							$missing .= " and ";
						else if (count($fileproblems) - $idx > 1)
							$missing .= ", ";
					}
					
					echo "<li>File $fname has a problem with its tag: $missing</li>\n";
	...
	/* GV: this function adds songs only when their last-modified time is newer than the one
	 *     registered in the database, or if they are not registered in the database in the first
	 *     place. the field that was added in the database is "last_modified" in the table
	 *     mp3act_songs. The function also logs its status to the mp3act_state table since it
	 *     now runs completely asynchronously from the UI.
	 */
	function insertSongs() {
			...
			foreach ($this->fileList as $path => $files) {
				if (count($files))
				{
					foreach ($files as $song) {
						...
						{		
							$this->outputProgressJavascript((int)(($cur_fileno*100)/$num_files));
							$last_timestamp = $current_timestamp;
						}
		
						$i += $this->importOneSong($getID3, "$path$song");
					}				
				}
			}
		}
		...
		$this->outputProgressJavascript(100);

		$time2 = time() - $time;
		echo "<li>Import took ".$time2." secs</li>\n";
		if ($this->displayResults == 1) {
			if ($i == 0) {
				echo "No New Songs Added to Database<br/>\n";
			}
		}
