...
$sql = "CREATE INDEX ON mp3act.mp3act_songs (filename(128))";
if (!mysql_query($sql)) {
  $errors = TRUE;
}

$sql = "ALTER TABLE mp3act_songs ADD last_modified int(11)";
if (!mysql_query($sql)) {
  $errors = TRUE;
}

$sql = "ALTER TABLE mp3act_settings ADD jukemode varchar(16)";
if (!mysql_query($sql)) {
  $errors = TRUE;
}

$sql = "ALTER TABLE mp3act_settings ADD mpdserver varchar(255)";
if (!mysql_query($sql)) {
  $errors = TRUE;
}

$sql = "ALTER TABLE mp3act_settings ADD mpdport varchar(16)";
if (!mysql_query($sql)) {
  $errors = TRUE;
}
