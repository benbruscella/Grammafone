...
function mpd_connect()
{
	...
	if (getSystemSetting("jukemode") == "mpd")
	{
		if ($mpd == null)
		{
			...
			$pass = getSystemSetting("mpdpass");
			
			if ($pass == "") $pass = NULL;
			$mpd = new mpd($server,$port, $pass);
...
function getCurrentSong($curArtist, $curSong){
	...
	if (inMpdMode())
	{
		if (mpd_connect() == 0) {
			$err = $mpd->errStr;
			$data[] = "<strong>Can't connect to MPD server</strong><br/><span id='artist'>$err</span><span id='song'></span>";
			$data[] = 0;
			$data[] = 0;
			return $data;
		}
...
function buildBreadcrumb($page,$parent,$parentitem,$child,$childitem){
	...
	if (inMpdMode())
	{
		...
		else if ($page == 'search' && $child != '') {
			$output = "<a href=\"#\" onclick=\"updateBox('search','0'); return false;\">Search</a> &#187; ";
		}
		switch($child){
			case 'album':
...
function editSettings($update,$invite,$downloads,$amazonid,$upload_path,$sample_mode,$mp3bin,$lamebin,$phpbin,$jukemode,$mpdserver,$mpdport, $mpdpass){
		mp3act_connect();
		if($update){
			...
			$query = "UPDATE mp3act_settings SET invite_mode=$invite,sample_mode=$sample_mode,downloads=$downloads,amazonid=\"$amazonid\",upload_path=\"$upload_path\",mp3bin=\"$mp3bin\",lamebin=\"$lamebin\",phpbin=\"$phpbin\",jukemode=\"$jukemode\",mpdserver=\"$mpdserver\",mpdport=\"$mpdport\",mpdpass=\"$mpdpass\" WHERE id=1";
			mysql_query($query);
    	...
    	$output .= "<strong>Amazon API Key</strong> <a href='http://www.amazon.com/webservices/' target='_new'>Obtain Key</a><br/><input type='text' size='30' name='amazonid' value='$row[amazonid]' /><br/><br/>\n";
    	 ...
    	 $output .= "<strong>MPD password</strong><br/><input type='password' size='30' name='mpdpass' value='$row[mpdpass]' /><br/><br/> ";
	...
	// JUKEBOX MODE -- only allowed if access level >= 7
		...
		if (accessLevel(7))
		{
			if (inMpdMode())
			{
				...
				// to do random play in MPD mode is not trivial, since MPD has no
				// such function. Therefore the function FindRandom implements a
				// search, which filters the results in a random way. See mpd.class.php				
				switch ($type) {
					case 'artists':
						 $artists = $mpd->GetArtists();
						 ...
						 if (count($items2) > 0) {
						 	...
						 	foreach($items2 as $item) {
						 		...
						 		$artist = $artists[$item];
						 		$res = $mpd->FindRandom($num, $numitems==$curitem, MPD_SEARCH_ARTIST, $artist, "", $numresults, $bins);
					...
					case 'albums':
						 ...
						 if (count($items2) > 0) {
						 	...
						 	foreach($items2 as $item) {
						 		...
						 		$res = $mpd->FindRandom($num, $numitems==$curitem, MPD_SEARCH_ARTISTALBUM, $album, $artist, $numresults, $bins);
						 		$numresults = $res['numresults'];
						 		$bins = $res['bins'];
						 	}
						 }
					...
					case 'genre':
						if (count($items2) > 0) {
						 	...
						 	foreach($items2 as $item) {
						 		...
						 		$res = $mpd->FindRandom($num, $numitems==$curitem, MPD_SEARCH_GENRE, $item, "", $numresults, $bins);
						 		$numresults = $res['numresults'];
						 		$bins = $res['bins'];
						 	}
						}
						break;
					case 'all':
				 		$res = $mpd->FindRandom($num, true, MPD_SEARCH_ALL);
				 		$numresults = $res['numresults'];
				 		$bins = $res['bins'];
						break;
				}
