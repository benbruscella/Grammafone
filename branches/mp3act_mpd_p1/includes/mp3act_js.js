    	...
    	function newWindow(type,id){
    		if(type == 'add')
    			newwindow = window.open('browse.php','addmusic','height=400,width=500,scrollbars=yes,resizable=yes');
    	...
    	function switchMode_cb(new_data){
    		if (new_data.substring(0,3) == "mpd") {
    			// jukebox is MPD
    			// note that the string is ALWAYS prepended with "mpd"
    			// in case the jukebox mode is set to mpd, even in case
    			// of a switch to streaming mode (the string will be
    			// 'mpdstreaming') since we need to detect we were
    			// switching to / from an MPD mode.. and in that case
    			// we need to reset the interface (since the song IDs
    			// etc are no longer valid).
    			new_data = new_data.substring(3);
    	...
    	function refresh_cb(new_data) {
			...
			function updateBox(type,itemid){
				...
				x_musicLookup(oldtype,olditemid,updateBox_cb);
			...
			function playlist_shuffle() {
				x_playlist_shuffle(playlist_shuffle_cb);	
			}

			function playlist_shuffle_cb() {
	    		x_playlistInfo(plinfo_cb);
	    		x_viewPlaylist(viewPlaylist_cb);
			}
			...
			function play_cb(new_data){
				// in case we're in jukebox mode, refresh the playlist after
				// doing a play
				x_playlistInfo(plinfo_cb);
		    	x_viewPlaylist(viewPlaylist_cb);
			...
			function mpd_updatedb_cb() {
				document.getElementById("loading").style.display = 'none';
				refresh();
			}
			
			function mpd_updatedb() {
				document.getElementById("loading").style.display = 'block';
				x_mpdUpdateDb(mpd_updatedb_cb);
			}
	      ...
	      if(form!=""){
			...
			function editSettings_cb(new_data){
				if(new_data == 1){
					...
					// jukebox setting changed. now better reset the frontend!
					init();
				}
			}
			...
			function editSettings(form){
				if(form != 0){
					...
					for (i=0;i<form.jukemode.length;i++){
						if (form.jukemode[i].checked == true)
							jukemode = form.jukemode[i].value;
					}
					
					x_editSettings(1,form.invite.value,form.downloads.value,form.amazonid.value,form.upload_path.value,form.sample_mode.value,form.mp3bin.value,form.lamebin.value,form.phpbin.value,jukemode, form.mpdserver.value, form.mpdport.value, form.mpdpass.value, editSettings_cb);
				}
				else{
