...
class addMusic {
  ...
  function &setFileList($filePath, $fileName, &$filelist) {
	$ret = &$filelist;

	$ret[$filePath][] = addslashes($fileName);
  ...
  function getID3Data($filenamepath, &$goodData) {
	
	  $getID3 = new getID3;
	  $tagInfo=array();
	  $tagInfo = $getID3->analyze($filenamepath);
	  ...
	  if(!( $goodData["artist"] = addslashes(ucwords($tagInfo['tags']['id3v2']['artist'][0])) )){
	    if(!($goodData["artist"] = addslashes(ucwords($tagInfo['tags']['id3v1']['artist'][0])) )){
	      ...
	      $errors .= print_r($tagInfo);
	      $flag=1;*/
		    $goodData["artist"] = "unkown";
	    }
	  }
	  #get Album
	  ...
	  if(!( $goodData["track"] = $tagInfo['tags']['id3v2']['track'][0] )){
		...
		if(!( $goodData["track"] = $tagInfo['tags']['id3v2']['track_number'][0] )){
		$goodData["track"] = 0;
		}
	    }
	  }
  ...
  function insertSongs() { 
	...
	set_time_limit(20);
      ...
      $tagInfo=array();
	    $tagInfo = $getID3->analyze(stripslashes($path)."".stripslashes($song));
	     ...
	     $query = "SELECT * FROM grammafone_songs WHERE filename=\"".$path.$song."\"";
	...
	if($flag==0){
		...
		if(substr($goodData['artist'], 0, 4) == "The "){
	     ...
	     $query = "SELECT artist_id FROM grammafone_artists WHERE artist_name=\"$artist\"";
		...
		$query = "INSERT INTO grammafone_artists VALUES (NULL,\"$artist\",\"$prefix\")";
	    ...
	    $query = "SELECT album_id FROM grammafone_albums WHERE album_name=\"$goodData[album]\" AND artist_id=$current_artist";
	    $result = mysql_query($query);
		...
		$query = "INSERT INTO grammafone_albums VALUES (NULL,\"$goodData[album]\",$current_artist,\"$goodData[genre]\",$goodData[year],\"\")";
		
		$result = mysql_query($query);
		$current_album = mysql_insert_id();
			}           
	    $query = "INSERT INTO grammafone_songs VALUES";
  ...
  function updateStats(){
    ...
    $query = "SELECT COUNT(DISTINCT album_id) as num_albums, 
	      ...
	      FROM grammafone_songs"; 
    $result = mysql_query($query);
    $row = mysql_fetch_assoc($result);
    $query="DELETE FROM grammafone_stats";
    mysql_query($query);
    $query2 = "SELECT COUNT(genre_id) as num_genres FROM grammafone_genres";
     $result2= mysql_query($query2);
    $row2 = mysql_fetch_assoc($result2);
    
    $query3="INSERT INTO grammafone_stats VALUES ( 
	    ".$row['num_artists'].", 
	    ".$row['num_albums'].",
	    ".$row['num_songs'].",
	    ".$row2['num_genres'].",
  ...
  function setFormattedFiles($files) {
	 ...
	 * that with one.
	 */
	$query = 'select album_num from grammafone_music where album = "' . $goodData['album'] . '" and artist = "' . $goodData['artist'] . '" LIMIT 1'; 
	$result = mysql_query($query);
	$result  = mysql_fetch_array($result);
	if ($result == null) {
	  $query = "SELECT MAX(album_num)as count FROM grammafone_music";
	  $result = mysql_query($query);
	...
	$query = "SELECT * FROM grammafone_music WHERE filename=\"".$song."\"";
	if(mysql_num_rows(mysql_query($query)) == 0){
	  $query = "INSERT INTO grammafone_music  (id,artist,album,album_num,track,name,genre,length,size,bitrate,year,date_entered,date_lastplay,filename,numplays,signature,comment,random) ";
